// =============================================
// File: backend/src/templates/invoice.ts
// Clean HTML template for invoice PDF generation
// - Fixes TS parse errors (proper template string)
// - Adds helpers (escape, currency formatting)
// - Safe optional fields (tax/discount/dates)
// =============================================
...
export type InvoiceTemplateInput = {
  ...
  summary: { subtotal: number; tax?: number; discount?: number; total: number; invoiceNumber?: string };
  meta?: { issueDate?: Date | string; dueDate?: Date | string };
};

const PKR = new Intl.NumberFormat("en-PK", { style: "currency", currency: "PKR", maximumFractionDigits: 0 });
const fmt = (n?: number) => PKR.format(Number(n ?? 0));
...
export function invoiceHtmlTemplate(data: InvoiceTemplateInput): string {
    ...
    <div class="card" style="margin-top:16px">
      <table>
        ...
        <tbody>
          ${data.items
            .map(
              (i) => `<tr><td>${esc(i.name)}</td><td class="right">${esc(i.qty)}</td><td class="right">${esc(fmt(i.price))}</td><td class="right">${esc(fmt(i.total))}</td></tr>`
            )
            .join("")}
        </tbody>
        <tfoot>
